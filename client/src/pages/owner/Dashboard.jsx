...
function Dashboard() {
  ...
  const dashboardCards = [
    { title: "Totel Cars", value: data.totalCars, icon: assets.carIconColored },
  ...
  return (
    <div className='px-4 pt-10 md:px-10 flex-1'>
          ...
          dashboardCards?.map((card, index) => (
            <div key={index} className='flex gap-2 items-center justify-between p-4 rounded-md border border-borderColor'>
              <div className=''>
                ...
                <p className='text-left font-semibold'>{card?.value}</p>
              </div>
              <div className='flex items-center justify-center w-10 h-10 rounded-full bg-primary/10'>
                <img src={card?.icon} alt='card_icon' className='h-4 w-4' />
              </div>
            </div>
          ))
        }
      ...
      <div className='flex flex-wrap items-start gap-6 mb-8 w-full'>
        {/* Recent Booking */}
        <div className='p-4 border border-borderColor rounded-md max-w-lg w-full'>
          <h1 className='text-lg font-medium'>Recent Bookings {data?.recentBooking?.length}</h1>
              ...
              <div key={index} className='mt-4 flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                  <div className='hidden md:flex items-center justify-center w-12 h-12 rounded-full bg-primary/10'>
                    <img src={assets?.listIconColored} alt='colore icon' className='h-5 w-5' />
                  </div>
                  <div className=''>
